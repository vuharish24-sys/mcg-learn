...
**Feature: logging out of a launched WordPress session now also logs out of MCG-Learn** — same rationale as the back-link (a shared/public computer left half-logged-in on the MCG-Learn side is a real risk). A `logout_redirect` filter on the same `mcglearn_return_url` cookie sends the browser to a new `GET /api/v1/auth/logout` route (signs out the Supabase session server-side, redirects to `/login`) instead of WordPress's own default. Requires `MCGLEARN_APP_URL` in `wp-config.php` (added by `onlineclass`). **Not fully verified yet**: the WordPress-side session and cookie both correctly cleared on logout (confirmed — the bar disappeared, toolbar stayed hidden), but the browser landed on WordPress's own `wp-login.php?redirect_to=.../wp-admin/&reauth=1` instead of reaching MCG-Learn's logout route — the `logout_redirect` filter isn't taking effect as written. Flagged to `onlineclass` for server-side log/config inspection, since this can't be debugged without PHP execution visibility. Not a regression of anything that worked before today.
