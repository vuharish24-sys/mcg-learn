...
**New feature, built in response to the user asking "how does a student get back to MCG-Learn after a lesson"**: there was no way — not a bug so much as a genuine missing piece, confirmed by checking that no such mechanism existed on either side. Fixed: `tutorLmsService.getLaunchUrl` now passes a `return_url` (the feed item's own course page) through to the mu-plugin's `/auto-login-token` call; the plugin sets a short-lived (`DAY_IN_SECONDS`) `httponly` cookie on token consumption and a new `wp_footer` hook renders a fixed "← Back to MCG Learn" bar for the rest of that browser session — deliberately a cookie, not user meta, so it can never leak onto an admin's or another account's *future* WordPress sessions the way the enrollment collision above did. Verified live: the banner renders correctly, survives navigating into a real lesson page (not just the course landing page), and the earlier version's href construction had a real bug (see below) since fixed and reverified.

**Real bug found and fixed the same day**: the first live click on the banner crashed MCG-Learn with a client-side `SecurityError` — the href was `https://mcg-learn.netlify.app//feed/...` (double slash), which the browser parses as a protocol-relative URL to host `feed` and Next.js's router chokes on. Root cause: `NEXT_PUBLIC_APP_URL` has a trailing slash in production, and `appUrl()` never stripped it, so naive `${appUrl()}/path` concatenation broke — not just for this new `return_url`, but latently for every other existing `appUrl()` call site in the app (certificates, referrals, partner links, the jobs API, sitemap/robots). Fixed at the source in `src/lib/env.ts` rather than patching just this one call site.

**Second real bug, also found live**: the back-link bar and WordPress's own admin toolbar are both `position:fixed` at `top:0`; the bar's higher z-index silently sat on top of and ate clicks meant for the toolbar underneath, including its own Log Out link — discovered while trying to test the logout feature below. Fixed by hiding the WP admin toolbar entirely for launched sessions (a subscriber has no real use for wp-admin shortcuts anyway) and adding an explicit "Log out" link into the bar itself, so hiding the toolbar doesn't leave students with no way to log out.

**Feature: logging out of a launched WordPress session now also logs out of MCG-Learn** — same rationale as the back-link (a shared/public computer left half-logged-in on the MCG-Learn side is a real risk). A `logout_redirect` filter on the same `mcglearn_return_url` cookie sends the browser to a new `GET /api/v1/auth/logout` route (signs out the Supabase session server-side, redirects to `/login`) instead of WordPress's own default. Requires `MCGLEARN_APP_URL` in `wp-config.php` (added by `onlineclass`).

**Third real bug, found while first testing the logout feature**: the redirect landed on WordPress's own `wp-login.php?redirect_to=.../wp-admin/&reauth=1` instead of MCG-Learn's logout route, even though the `logout_redirect` filter's return value was correct. Root cause (found by `onlineclass` reading WordPress core's live source, not guessing): `wp_safe_redirect()` — what the logout handler uses to apply that return value — only allows redirecting to the current site's own host plus an explicit `allowed_redirect_hosts` allowlist; anything else is silently swapped for `wp_safe_redirect()`'s own fallback (`admin_url()`), which is what produced the observed URL once the user was already logged out. Fixed with one more filter deriving the allowed host from `MCGLEARN_APP_URL` itself, so it can't drift out of sync with a second hardcoded copy of the domain.

**Fully verified live, end to end**: purchased/granted access → launched into WordPress → clicked the bar's "Log out" link → landed on `mcg-learn.netlify.app/login` (not WordPress's login) → confirmed genuinely signed out of MCG-Learn by requesting `/dashboard` directly and being redirected to `/login?next=%2Fdashboard` rather than seeing the dashboard. All three real bugs from this feature (double-slash crash, banner/toolbar click-eating, off-site redirect rejection) are now fixed and confirmed working, not just deployed.

**Real gap found and deliberately deferred**: while re-testing the WordPress vars, a real ₹1,000 test payment (the user personally completed checkout, confirmed a genuine Razorpay success screen) never got recorded — the `Purchase` row stayed `PENDING` with no `razorpayPaymentId`, and the course still showed "Buy for ₹1,000" on return. Root cause: this app currently confirms every purchase entirely through a client-side callback that fires *after* Razorpay's checkout succeeds (`POST /api/v1/purchases/verify`); if the browser navigates away, closes, or hits any hiccup between payment success and that callback completing, the app never learns the payment happened — and `RAZORPAY_WEBHOOK_SECRET`, the documented safety net for exactly this case (`/api/v1/webhooks/razorpay`, code already exists and is correct), was never configured on Netlify. Manually reconciled this specific stuck purchase (marked `PAID`, called `grantAccess` directly) since the charge was real; the user chose to set up the actual webhook later rather than in this session. **This is a real, live gap for any actual paying customer right now** — worth prioritizing before real course sales open.

**Also observed, not a bug in this app**: after logging out, the "Sample Course" content (curriculum + Lesson 1 text) was still fully visible to a logged-out, anonymous visitor — this specific placeholder course doesn't appear to have real content-gating configured on the Tutor LMS side. Worth checking when real paid course content is set up, since MCG-Learn's own access checks (Purchase/enrollment) are working correctly regardless — this would be a WordPress/Tutor LMS course-settings matter, not something today's integration controls.
