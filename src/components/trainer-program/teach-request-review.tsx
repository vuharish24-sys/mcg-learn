...
export function TeachRequestReview({
  requestId,
  ...
  async function decide(body: Record<string, unknown>) {
    setBusy(true);
    setError("");
    const response = await fetch(`/api/v1/module-teach-requests/${requestId}/decide`, {
      method: "POST",
    ...
    return (
      <div className="space-y-2 rounded-lg border p-3 dark:border-slate-800">
        ...
        <div className="flex gap-2">
          <Button
            size="sm"
            disabled={busy || !rateRupees}
            onClick={() => decide({ status: "APPROVED", finalCompensationType: compensationType, finalRateAmountPaise: Math.round(Number(rateRupees) * 100) })}
          >
            Confirm approval
          </Button>
